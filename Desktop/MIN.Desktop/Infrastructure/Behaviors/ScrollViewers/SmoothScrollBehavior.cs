...
public class SmoothScrollBehavior : StyledElementBehavior<ScrollViewer>
{
    ...
    private const double ANIMATION_DURATION = 170; // длительность анимации в мс

    /// <summary>
    /// Свойство шагов прокрутки ползунка
    /// </summary>
    public readonly static DirectProperty<SmoothScrollBehavior, double> ScrollStepSizeProperty =
        AvaloniaProperty.RegisterDirect<SmoothScrollBehavior, double>($"{nameof(ScrollStepSize)}Property",
                                                                                      o => o.ScrollStepSize,
                                                                                      (o, v) => o.ScrollStepSize = v);
    /// <summary>
    /// Свойство изменения размера ползунка
    /// </summary>
    public readonly static StyledProperty<ChangeSize> ScrollChangeSizeProperty =
    ...
    private void OnPointerWheelChanged(object? sender, PointerWheelEventArgs e)
    {
        ...
        var step = double.Abs(newOffset.Y - scp!.Offset.Y);
        ...
        var offsetChanged = newOffset != scp.Offset;
    ...
    private void AnimateScroll(double delta)
    {
        ...
        if (isAnimating)
        {
            ...
            var elapsedTime = (currentTime - animationStartTime).TotalMilliseconds;
            var progress = double.Min(elapsedTime / ANIMATION_DURATION, 1.0);
    ...
    private Vector SnapOffset(Vector offset, Vector direction = default, bool snapToNext = false)
    {
        ...
        var areVerticalSnapPointsRegular = false;
